...
def __build_muffins(p, m, s):
    ...
    loose_upper_bound = ub['upper_bound']
    #logging.debug("{0} UB: {1}".format(ub['bound_type'], loose_upper_bound))
    rows.append([ [idx_min_sliver],
                  [1],
              ])
    senses.append("L")
    rhs.append(float(loose_upper_bound))
...
def solve(m, s, timelimit=300.0, verbose=False):
    ...
    try:
        ...
        return (build_ret['bound'], bound, bound_type, is_opt, solve_s)
